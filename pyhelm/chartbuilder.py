...
class ChartBuilder(object):
    ...
    def get_templates(self):
        ...
        for root, _, files in os.walk(os.path.join(self.source_directory,
            ...
            for tpl_file in files:
                ...
                templates.append(Template(name=template_name, data=self.read_file(os.path.join(root,tpl_file))))
    ...
    def read_file(self, path):
        # Use codecs.open() and set the expected encoding, ignoring illegal characters
        content = codecs.open(path, encoding='utf-8', errors='ignore').read()
        # codecs.open.read() gives us a string, so we need to convert to bytes
